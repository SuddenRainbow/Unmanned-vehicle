...
本仓库用于存放无人车（智能小车）实训过程中的 STM32 工程代码。

当前目录 **`跑马灯/`** 是一个基于 **STM32F103** 的 Keil MDK5 工程，实现了：

- **串口收发打印**：USART1 收发、`printf` 重定向、接收中断。
- **炫彩灯跑马灯**：驱动前后共 **12 颗 WS2812 可寻址 RGB 灯**（前灯 6 颗 + 尾灯 6 颗），跑马灯效果。
...
1. 用 Keil MDK5 打开 `跑马灯/USER/Template.uvprojx`。
...
- 学会了 **跑马灯**等循环/状态逻辑的编写，理解了「数据写入数组 → 刷新点亮」的编程思想。
