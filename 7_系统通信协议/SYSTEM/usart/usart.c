...
u8 CAR_buff[4];                     //解析出的数据帧: [0]方向A [1]速度A [2]方向B [3]速度B
u8 uart_rec_flag=0;                 //收到一帧完整数据的标志
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)   //判断中断类型
		{
			...
			if(USART_RX_BUF[0]==0xFC)      //找帧头
				USART_RX_COUNT++;
			else                          //否则就不要
				USART_RX_COUNT=0;
		  if(USART_RX_COUNT>6)USART_RX_COUNT=0;   //防错/越界保护

		  if(USART_RX_BUF[5]==0xFD)       //找到帧尾
			{
				USART_RX_COUNT=0;
				CAR_buff[0]=USART_RX_BUF[1];   //方向A
				CAR_buff[1]=USART_RX_BUF[2];   //速度A
				CAR_buff[2]=USART_RX_BUF[3];   //方向B
				CAR_buff[3]=USART_RX_BUF[4];   //速度B
